...
def plot_bcf(t, exact, approx, error):
    ...
    ax1.set_ylabel(r'$C(t)$', fontsize=ls1)
    ax2.set_ylabel(r'$\delta C(t)$', fontsize=ls1)
    ax2.set_xlabel(r'$t$ $(\mathrm{fs})$', fontsize=ls1)

    plt.subplots_adjust(left=0.18, right=0.95, top=0.95, bottom=0.11, hspace=0, wspace=0)
    plt.savefig('bcf1.png')
    #plt.show()

def plot_bcf_float(t, exact, approx, error):
    fig, axes = plt.subplots(2, 1, figsize=(8, 7), gridspec_kw={'hspace': 0.04, 'wspace': 0})
    ax1 = axes[0]
    ax2 = axes[1]
    ax1.plot(t, approx, linestyle='-', label='Approx', lw=lw1, color=color1)
    ax1.plot(t, exact, linestyle='--', label='Reference', lw=lw2, color=color3)
    ax2.plot(t, error, linestyle='-', lw=lw1, color=color1)
    ax1.legend(loc='upper right', fontsize=ls2)
